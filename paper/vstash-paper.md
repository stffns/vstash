...
*Published baselines from the MTEB SciFact leaderboard (https://huggingface.co/spaces/mteb/leaderboard, accessed April 2026) and the BEIR paper (Thakur et al., 2021). ColBERTv2 result from Santhanam et al. (2022). EmbeddingGemma result from vstash evaluation (google/embeddinggemma-300m, 768d, ONNX).*

**With EmbeddingGemma, vstash achieves NDCG@10 = 0.7801 on SciFact** — surpassing ColBERTv2 by 12.6%, BM25 by 17.3%, and dense-only by 19.5%. Upgrading from BGE-small (384d) to EmbeddingGemma (768d) yields +7.4% NDCG with only +1.5ms search latency, confirming that the embedding model is the primary quality ceiling — not the retrieval pipeline.
