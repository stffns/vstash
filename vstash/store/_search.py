...
class _SearchEngineMixin:
    ...
    def _mmr_dedup(
        ...
        # Precompute L2 norms lazily for cosine similarity to avoid O(K * N) recomputation.
        ...
        for _ in range(min(top_k, len(ranked))):
            ...
            # Bypass penalty updates if the selected chunk is the only one from its document
