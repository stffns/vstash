...
class _SearchEngineMixin:
    ...
    def _mmr_dedup(
        ...
        # Lazily evaluate L2 norms to avoid computing hypot for chunks that are
        # never compared against a sibling.
        ...
        for _ in range(min(top_k, len(ranked))):
            ...
            # Fast path: if this is the only chunk from this document, no siblings
            # to penalize, so we can skip the similarity update entirely.
