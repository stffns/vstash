...
class _SearchEngineMixin:
    ...
    def _mmr_dedup(
        ...
        # Lazily compute L2 norms for cosine similarity only when needed
        # (when chunks from the same document compete).
        ...
        for _ in range(min(top_k, len(ranked))):
            ...
            # Fast path: skip penalty updates if this is the only chunk from this doc
