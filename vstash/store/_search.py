...
class _SearchEngineMixin:
    ...
    def _mmr_dedup(
        ...
        # Track lazy L2 norms for cosine similarity to avoid eager O(N) computation
        # for chunks that are never compared against a sibling.
        ...
        for _ in range(min(top_k, len(ranked))):
            ...
            # Bypass penalty updates entirely if there are no other siblings
            if len(siblings) > 1:
