...
class _SearchEngineMixin:
    ...
    def _mmr_dedup(
        ...
        # Lazy evaluate L2 norms for cosine similarity to avoid computing norms for chunks that are never compared.
        ...
        for _ in range(min(top_k, len(ranked))):
            ...
            # Bypass similarity penalty updates entirely if the selected chunk is the only one from its document
