...
class _SearchEngineMixin:
    ...
    def _mmr_dedup(
        ...
        # Lazily evaluate L2 norms only when needed
        ...
        for _ in range(min(top_k, len(ranked))):
            ...
            # Optimization: If this chunk is the only one from its document,
            # there's no need to compute embeddings or sibling penalties.
            if len(siblings) <= 1:
                continue

            new_emb = chunk_embs[best_idx]
            if new_emb is not None:
                # Lazily compute norm for the newly selected chunk if needed
                ...
                for idx in siblings:
                    if in_remaining[idx]:
                        ...
                        if idx_emb is not None:
                            # Lazily compute norm for sibling
