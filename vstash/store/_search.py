...
class _SearchEngineMixin:
    ...
    def _mmr_dedup(
        ...
        # Lazily evaluate L2 norms only when needed for similarity comparisons
        # to avoid computing math.hypot for chunks without siblings or never selected.
        ...
        for _ in range(min(top_k, len(ranked))):
            ...
            # is the only one from its document (no siblings to penalize).
            if len(doc_indices) > 1:
                new_emb = chunk_embs[best_idx]
                if new_emb is not None:
                    # Lazily evaluate the newly selected embedding's norm
                    ...
                    for idx in doc_indices:
                        if in_remaining[idx]:
                            ...
                            if idx_emb is not None:
                                # Lazily evaluate the remaining sibling's norm
