...
```
vstash/
  __init__.py        # version (__version__ = "0.39.0")
...
- **Cross-document near-duplicate collapse (v0.39)** (#509): `dedup_threshold: float | None = None` on `VstashStore.search`, `services/search.py`, `Memory.search`, MCP `vstash_search`, CLI `vstash search --dedup`, and `federated_search` (applied **per profile**, before the cross-profile RRF merge -- cross-profile comparison would need embeddings from stores already closed by merge time). Drops a chunk whose cosine similarity to an already-kept, higher-ranked chunk is `>=` the threshold, **regardless of document** -- which `mmr_lambda` never does, since `_mmr_dedup` penalises only same-document siblings. Default `None` leaves ranking untouched, so the published BEIR / v2 / v3 numbers stand without revalidation. Runs after the recency boost (which re-sorts, and keep-first assumes final score order) and **before** MMR + the `top_k` cut, so a slot freed by a duplicate is refilled with distinct content instead of shortening the list. Compares each candidate against the vectors kept so far into a preallocated buffer rather than materialising an N x N gram matrix (1.6 GB -> 314 MB at 20k candidates, since the pre-MMR pool scales with `top_k`). The comparison carries a float32 tolerance (`_DEDUP_SIM_EPS`): the dot product of a vector with an identical copy lands on 0.99999994 often enough that a bare `>= 1.0` missed about half the byte-identical pairs. Same-document pairs are deliberately **not** exempt -- measured over four repo documents at the default 1024/128 chunking, adjacent chunks land at 0.59-0.87 cosine and no pair reached 0.95, so 0.95 is the recommendation and ~0.92 the floor. Motivating measurement + refutation of the candidate-pool hypothesis (#507, closed) in `experiments/probe_507_candidate_pool.py`.
- **Query cache key includes the filter tree (v0.39)** (#509): the #106 boolean `filters` tree was absent from `_compute_search_cache_key` and, unlike `exact_match`, did not skip the cache either. On a store with `[cache] query_cache_size > 0`, `search(q)` and `search(q, filters={"not": {...}})` shared one slot, so whichever ran first served the other its results. The tree is canonicalised to `sort_keys` JSON before hashing (nested dicts are unhashable); sibling dict order no longer fragments the cache, list order still does and costs at most a miss.
- **v0.39 correctness cluster**: (#423) the DB connection runs in autocommit with explicit `BEGIN IMMEDIATE`, so Python's implicit-transaction layer can no longer hold a write lock open across a read. (#424) `Memory.update(text=...)` replaces content atomically via `add_documents_batch`. (#425) transient HF ONNX init failures are no longer cached as permanent, and backend resolution is locked. (#426) IVFPQ dim-check on load, and a transient `COUNT` error no longer downgrades a fitted index. (#427) every result model is frozen -- the query LRU cache hands out shared objects, so a mutable `SearchResult` let one caller corrupt another's cached entry. (#428) the schema-version check runs *before* stamping, and the `created_at` backfill happens inside the ALTER transaction. (#432) `VstashRetriever` exposes the full `SearchResult` metadata on retrieved LangChain `Document`s.
- **v0.39 features**: (#429) Tier-0 content-hash dedup on `remember` -- a byte-identical re-ingest is a NOOP instead of a re-chunk + re-embed. (#430) `replace_document_content` store primitive with row discovery inside the transaction. (#433) explicit `project` override on `journal_save` / `journal_recall` / `journal_log` / `journal_prune`.
- **v0.39 perf**: (#420) cheaper telemetry prune + bisect histogram bucketing on the hot path. (#504) `_apply_recency_boost` drops its secondary query and sorts in place.
