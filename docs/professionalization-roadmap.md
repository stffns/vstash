...
3. **Coverage reporting** -- configure pytest-cov flags in `pyproject.toml` (`[tool.pytest.ini_options]` or `[tool.coverage.run]`) so local and CI runs share the same config, then add a Codecov badge. Does not need to gate merges; reporting alone catches coverage erosion in new PRs. `test` job already pulls pytest-cov via `[dev]`.
...
14. **Docs site via mkdocs-material** -- `docs/` has accumulated nearly twenty markdown files with zero navigation. mkdocs-material with the `gh-pages` deploy workflow gets `docs.vstash.dev` (or a readthedocs subdomain) for free. Signal to paper readers that "local memory" is a product, not a gist.
...
16. **Typed strictness on public API** -- P0 adds `mypy` to pre-commit for basic checking, and the `dev` extras already pull it. This item is the next step up: enable strict mode for `vstash/` (public API only), keep the rest permissive. Most signal for the least ceremony.
