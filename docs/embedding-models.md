...
| `Stffens/bge-small-rrf-lme-v1` | 384 | ~700 ch/s | Domain-specific | Chat-memory specialist. Trained on 398 labeled LongMemEval queries via the gated retrain loop. +3.79pp R@5 (95% CI [+1.72, +6.19]) on n=102 holdout vs vanilla BGE-small. **Use when your corpus is primarily chat sessions / agent memory.** Not recommended for general BEIR-style retrieval; this is a domain-specialised model, not a universal embedder. See [paper v2 Section 6](../paper/v2/vstash-paper-v2.md) for the case study. |
| `BAAI/bge-base-en-v1.5` | 768 | ~300 ch/s | Excellent | |
| `nomic-ai/nomic-embed-text-v1.5` | 768 | ~300 ch/s | Excellent | |

> **Current pick for general retrieval (2026-04-19):** `Stffens/bge-small-rrf-v3` is a BGE-small model fine-tuned with vstash's own hybrid retrieval disagreement signal using the H-R9 winning config (`temperature=0.5, total_triples=60000`). Same 384 dims, same speed as the base model; macro NDCG@10 0.6405 across SciFact + NFCorpus + SciDocs + FiQA + ArguAna (vs 0.6246 for v2, 0.6118 for base). Both v2 and v3 beat ColBERTv2 on all 5 BEIR datasets under this pipeline. Switch in with `vstash reindex --model Stffens/bge-small-rrf-v3`, or fine-tune on your own data via `vstash retrain` (see [docs/retrain.md](retrain.md)).
>
> **Current pick for chat memory (2026-04-27):** `Stffens/bge-small-rrf-lme-v1` is the eval-gated chat-memory specialist (LongMemEval, n=102 holdout +3.79pp R@5 over vanilla). v3 is BEIR-tuned and *regresses* on chat memory by -2.45pp NDCG@10; if your corpus is conversational, prefer lme-v1 or run your own gated retrain (see [docs/retrain.md](retrain.md)). This split is the headline finding of paper v2 -- benchmark-quality retrieval does not transfer across domains, so per-domain models beat universal embedders.
