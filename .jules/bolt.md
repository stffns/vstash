...
## YYYY-MM-DD - Lazy L2 Norms in MMR
**Learning:** In MMR deduplication, computing `math.hypot` for all candidates is often a waste because many chunks never get compared against a sibling. Skipping updates when `len(doc_indices) <= 1` further eliminates redundant work for single-chunk documents.
**Action:** Evaluate properties lazily on the hot path only when they are needed for an operation, and short-circuit operations when no work needs to be done.
