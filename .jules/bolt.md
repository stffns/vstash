...
## YYYY-MM-DD - N+1 database round-trips in post-processing
**Learning:** In batched post-processing steps like _apply_recency_boost, performing secondary SQLite queries to fetch metadata (e.g., created_at) introduces unnecessary N+1 query overhead. The required data is already fetched in the initial candidate generation queries.
**Action:** Pre-fetch necessary properties in the initial SQL queries and carry them through the processing pipeline in dictionaries to avoid N+1 database round-trips.
