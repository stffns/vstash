...
## YYYY-MM-DD - Lazy L2 norm computation in MMR deduplication
**Learning:** Precomputing L2 norms for all chunks in MMR deduplication takes O(K * N) where N is the pool size. Most chunks are from single-chunk documents and never trigger the MMR penalty loop.
**Action:** Lazily calculate L2 norms only for chunks that share the same document and actually enter the similarity penalty calculation. Additionally, bypass the penalty loop entirely for documents with only one candidate chunk.
