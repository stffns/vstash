...
## 2024-05-20 - Lazy evaluation and loop bypass in MMR deduplication
**Learning:** In MMR deduplication, computing L2 norms (e.g., `math.hypot`) for all candidates ahead of time incurs an O(N) penalty even when those candidates are never selected or are the only chunks from their document.
