...
**Learning:** In MMR deduplication, eagerly precomputing L2 norms (`math.hypot`) for all candidates is inefficient since many chunks are never compared if their document only has a single returned chunk or if the selection terminates early.
