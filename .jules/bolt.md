...
## 2024-05-20 - Pre-fetch database properties to avoid N+1 queries
**Learning:** Pre-fetching necessary properties like `created_at` in initial SQL candidate generation queries and carrying them through the pipeline in dictionaries avoids expensive batched queries or N+1 round-trips in post-processing steps (like `_apply_recency_boost`).
**Action:** When a post-processing stage needs column data for scoring or filtering, join and retrieve that column in the initial `SELECT` rather than performing subsequent queries.
