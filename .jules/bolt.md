...
## 2024-07-26 - Lazy L2 Norm Evaluation
**Learning:** In MMR deduplication, eagerly computing L2 norms via `math.hypot` for all embedding candidates wastes CPU cycles, as most chunks are never compared against siblings.
**Action:** Use a boolean mask and initialize L2 norms as `None`, only lazily evaluating them (`math.hypot`) when a chunk is actually compared against a sibling. Also, completely skip similarity penalty updates if the selected chunk has no siblings (`len(doc_to_indices[new_doc_key]) <= 1`).
