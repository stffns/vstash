...
## YYYY-MM-DD - Lazily evaluate L2 norms in MMR dedup
**Learning:** In MMR deduplication, eagerly computing L2 norms (`math.hypot`) for all candidates before selection causes unnecessary computations because many chunks never get selected or have no siblings to compare against.
**Action:** Apply lazy evaluation to `chunk_norms` such that norms are calculated only when a sibling similarity penalty is being updated, and skip similarity penalty logic completely if the selected chunk is the only one from its document (`len(doc_indices) <= 1`).
