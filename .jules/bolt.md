...
## 2024-07-29 - MMR dedup lazy L2 norms and sibling skips
**Learning:** In MMR deduplication, calculating the L2 norm for every single chunk ahead of time is wasteful since most chunks won't have intra-document siblings and thus will never require a cosine similarity calculation for the redundancy penalty.
**Action:** Use lazy evaluation for L2 norms (`math.hypot`) and skip similarity penalty calculations entirely if the chosen document chunk has no siblings in the remaining set.
