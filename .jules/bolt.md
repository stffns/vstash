...
## 2024-07-09 - Lazy evaluation in MMR deduplication
**Learning:** Eagerly precomputing L2 norms for all candidates in MMR deduplication (`_mmr_dedup`) wastes computational cycles when many chunks do not need their norm calculated, as they may never be compared against a selected sibling chunk.
**Action:** In MMR deduplication, lazily evaluate L2 norms (e.g., `math.hypot`) only when a chunk is actually compared against a selected sibling. Additionally, bypass similarity penalty updates entirely if the selected chunk is the only one from its document (`len(doc_indices) <= 1`).
