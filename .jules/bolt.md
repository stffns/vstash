...
## 2024-05-20 - Lazy computation of cosine L2 norms in MMR
**Learning:** During MMR deduplication, computing L2 norms (via `math.hypot`) for all selected chunks eager-loads an $O(N)$ overhead which is heavily unused when chunks have no siblings in the results or never get compared.
**Action:** Replace the eager L2 norms list generation with a lazy initialization logic where `math.hypot` is calculated only when explicitly needed in the sibling penalty update loop. Also bypass similarity penalty updates completely when `len(doc_indices) <= 1`.
